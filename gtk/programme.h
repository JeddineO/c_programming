...
void createcomboboxitems(xmlNodePtr node, ComboBox *combobox)
{
    for (xmlNodePtr child = node->children; child != NULL; child = child->next)
    {
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "item") == 0)
            add_element(&(combobox->items), (char *)xmlGetProp(child, (xmlChar *)"label"));
    }
}
...
MaFenetre *Init_default_window_prop(MaFenetre *wind)
{
    ...
    wind->size.x = 450;
    wind->size.y = 250;
    wind->title = (gchar *)"default title";
...
void linkwidgets(GtkWidget *parent, GtkWidget *child)
{
    ...
    if (GTK_IS_SCROLLED_WINDOW(parent))
        gtk_container_add(GTK_SCROLLED_WINDOW(parent), child);
}
    ...
    for (xmlNodePtr child = node->children; child != NULL; child = child->next)
    {
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "menu") == 0)
        {
            ...
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), menu->menu, menu->pos.x, menu->pos.y);
        }
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "box") == 0)
        {
            ...
            fils = box->box;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), box->box, box->pos.x, box->pos.y);
            CreateWidgets(child, box->box);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "window") == 0)
        {
            ...
            fils = wind->wind;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), wind->wind, wind->pos.x, wind->pos.y);
            CreateWidgets(child, wind->wind);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "simplebutton") == 0)
        {
            ...
            fils = buttonsimple->button;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), buttonsimple->button, buttonsimple->pos.x, buttonsimple->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "checkbutton") == 0)
        {
            ...
            fils = buttoncheck->button;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), buttoncheck->button, buttoncheck->pos.x, buttoncheck->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "radiobutton") == 0)
        {
            ...
            fils = buttonradio->button;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), buttonradio->button, buttonradio->pos.x, buttonradio->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "image") == 0)
        {
            ...
            fils = image->image;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), image->image, image->pos.x, image->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "label") == 0)
        {
            ...
            fils = label->label;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), label->label, label->pos.x, label->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "textview") == 0)
        {
            ...
            fils = view->textview;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), view->textview, view->pos.x, view->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "scale") == 0)
        {
            ...
            fils = scale->scale;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), scale->scale, scale->pos.x, scale->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "entry") == 0)
        {
            ...
            fils = entry->entry;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), entry->entry, entry->pos.x, entry->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "grid") == 0)
        {
            ...
            create_grid_items(child, gridd);
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), gridd->grid, gridd->pos.x, gridd->pos.y);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "fixed") == 0)
        {
            ...
            fils = fixed->fixed;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), fixed->fixed, fixed->pos.x, fixed->pos.y);
            CreateWidgets(child, fixed->fixed);
        }
        ...
        if (child->type == XML_ELEMENT_NODE && strcmp((char *)child->name, "combo") == 0)
        {
            ...
            fils = combo->combo;
            if (GTK_IS_FIXED(parent))
                gtk_fixed_put(GTK_FIXED(parent), combo->combo, combo->pos.x, combo->pos.y);
        }
...
void parsefile(const char *filename)
{
    ...
    if (root->type == XML_ELEMENT_NODE && strcmp((char *)root->name, "window") == 0)
    {
        ...
        fenetre(wind);
        if (strcmp((char *)wind->scroll, "auto") == 0 || strcmp((char *)wind->scroll, "always") == 0)
            CreateWidgets(root, wind->scrollwind);
        else

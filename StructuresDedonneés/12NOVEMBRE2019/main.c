...
int Insert_Pos(MonTableau *ST, int valeur, int pos)
{
    ...
    if (Tab_Est_Saturee(*ST))
        return ((int)0);
    for (indice = ST->Nbelem; indice >= 0; indice--)
    {
...
Cellule *MinElem(Cellule *l, int *Min)
{
    *Min = l->info;
    Cellule *ptr;
    ptr = l;
...
Cellule *TransPileListe(Cellule *l, Cellule *p)
{
    int *Min;
    Cellule *NE;
    while (p)
...
//    char phrase[10];
//    phrase[10]="1a";
//    phrase[10]=Trans_Phrase(phrase[10]);
//    for(int i=0;i<=10;i++)
//    printf("%c",phrase[i]);
    return 0;
}
